...
function exportAnnotationsToLPTXT(output) {
    ...
    // 保存新文档到用户文件夹
    var tempFilePath = app.getPath("user") + "/"+ docName + "_字数统计.pdf"; // 使用用户文件夹路径
...
// 统计output中的字符数，去掉空行空格等
var charCount = output.replace(/\s+/g, '').length;
var charCountmun = "总字数: " + charCount + "\n" +output;
exportAnnotationsToLPTXT(charCountmun);
