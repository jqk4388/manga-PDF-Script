...
# 检查是否在<translate_input>标签内
def check_translate_tag(s: str) -> bool:
    """检查响应是否包含在<translate_input>标签内"""
    pattern = r'<translate_input>.*?</translate_input>'
    return bool(re.search(pattern, s, re.DOTALL))

# 提取<translate_input>标签内的内容
def extract_translate_content(s: str) -> str:
    """提取<translate_input>标签内的内容"""
    pattern = r'<translate_input>(.*?)</translate_input>'
...
def check_multiline_text(source_text_dict, translated_dict):
    ...
    Args:
        source_text_dict (dict): 源文本字典，key为字符数字，value为文本。
        translated_dict (dict): 输入的字典，key为字符数字，value为文本。
...
# 检查AI返回的列表中的原文与实际原文是否一致
def check_list_original_text(source_text_dict, response_str):
    """
    检查AI返回的列表格式响应中的原文与实际原文是否一致
    
    Args:
        source_text_dict (dict): 实际原文字典
        response_str (str): AI返回的响应字符串
    
    Returns:
        bool: 如果列表中的原文与实际原文一致，返回True，否则返回False
    """
    # 提取响应中的原文行
    import re
    original_lines = []
    ...
    # 检查提取的原文行数是否与实际原文行数一致
    if len(original_lines) != len(source_text_dict):
        return False
    
    # 检查提取的原文内容是否与实际原文内容一致
    source_lines = [v for k, v in sorted(source_text_dict.items(), key=lambda x: int(x[0]))]
    
    for i, (source, extracted) in enumerate(zip(source_lines, original_lines)):
        # 去除首尾空格后比较
        if source.strip() != extracted.strip():
            return False
...
class ResponseChecker():
    ...
    def check_response_content(self, response_str, response_dict, source_text_dict, source_lang):
        """
        检查翻译响应内容的完整性和正确性

        Args:
            response_str (str): 原始响应字符串
            response_dict (dict): 解析后的响应字典
            source_text_dict (dict): 源文本字典
            source_lang (str): 源语言
        ...
        # 检查是否包含在<translate_input>标签内
        if not check_translate_tag(response_str):
            error_content = f"响应未包含在<translate_input>标签内，回复内容：\n{response_str}"
            return False, error_content

        # 检查列表格式响应中的原文是否与实际原文一致
        if '-原文' in response_str:
            if not check_list_original_text(source_text_dict, response_str):
                return False, "【原文不一致】 - AI返回的列表中的原文与实际原文不一致"
